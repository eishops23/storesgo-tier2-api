...
// Check for wrong table name — production uses "categories" not "taxonomy"
if (code.match(/FROM taxonomy\b/) || code.match(/INTO taxonomy\b/)) {
  issues.push('Uses "taxonomy" table but production DB uses "categories" — will crash with "relation taxonomy does not exist"');
} else {
  passed.push('Uses "categories" table name (correct for production)');
}
