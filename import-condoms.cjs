 ...
 * =====================================================================
 * Creates: user → seller → store → taxonomy categories → 58 products
 * 
 ...
 *   - taxonomy: (name, slug, parent_id, sortOrder)
 ...
 *
 * Safe to re-run: uses ON CONFLICT for seller/store/taxonomy/products
 */
...
// ═════════════════════════════════════════════════════════════════════
// PRODUCT CATALOG — 58 PRODUCTS (36 existing + 22 new)
...
async function main() {
  ...
  if (!DRY_RUN) {
    await client.query(`
      ...
      ON CONFLICT ON CONSTRAINT stores_slug_key DO UPDATE SET "isActive" = true, "updatedAt" = NOW()
  ...
  // ── STEP 4: Create taxonomy categories ──
  log('Step 4: Creating taxonomy categories...');

  // Get Health & Wellness parent ID
  const hwRow = await client.query(`SELECT id FROM taxonomy WHERE slug = 'health-and-wellness'`);
  if (hwRow.rows.length === 0) {
    log('  ERROR: health-and-wellness category not found! Aborting.');
    await client.end();
    return;
  }
  ...
  if (!DRY_RUN) {
    await client.query(`
      INSERT INTO taxonomy (name, slug, parent_id, "sortOrder", "createdAt", "updatedAt")
      VALUES ('Sexual Wellness', 'sexual-wellness', $1, 0, NOW(), NOW())
      ON CONFLICT (slug) DO NOTHING
    `, [hwId]);
  }
  const swRow = await client.query(`SELECT id FROM taxonomy WHERE slug = 'sexual-wellness'`);
  let swId;
  ...
  const cRow = await client.query(`SELECT id FROM taxonomy WHERE slug = 'condoms'`);
  let categoryId;
  ...
  for (const p of PRODUCTS) {
    try {
      ...
      const tags = [p.brand.toLowerCase(), p.material.toLowerCase().replace(/[^a-z0-9]+/g, '-'), 'condoms', 'sexual-wellness', 'HSA', 'FSA'].join(',');

      const result = await client.query(`
        INSERT INTO products (
          name, slug, description, sku, "externalId",
          "priceCents", "imageUrl", source_image_url, currency,
