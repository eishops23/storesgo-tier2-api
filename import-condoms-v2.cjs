 ...
 *   - categories: (name, slug, parent_id, sortOrder)
...
async function main() {
  ...
  const hwRow = await client.query(`SELECT id FROM categories WHERE slug = 'health-and-wellness'`);
  ...
  if (!DRY_RUN) {
    await client.query(`
      INSERT INTO categories (name, slug, parent_id, "sortOrder", "createdAt", "updatedAt")
